...
func ParseWildcard(expr string) (bool, error) {
	...
	if len(tokens) >= 2 && strings.EqualFold(tokens[1], "AS") {
		...
		return false, fmt.Errorf("wildcard cannot be aliased: %q", expr)
	}
	...
	return false, fmt.Errorf("invalid wildcard syntax: %q", expr)
}
